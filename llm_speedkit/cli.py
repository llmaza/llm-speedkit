...
from llm_speedkit.core.stats import pctl, parse_csv_ints, parse_csv_strs
...
# NEW: infer bench subtree (for schema + later infer bench)
...
# -----------------------------
# Row writing / printing (existing infer_runs.*)
# -----------------------------
def _write_row(cfg: InferConfig, env: Dict[str, Any], result: Dict[str, Any], elapsed: float) -> Dict[str, Any]:
    ...
    return {
        ...
        # keep vLLM knobs for traceability
        "vllm_tp": cfg.vllm_tp,
...
def bench_smoke(
    ...
    """
    Writes 1 row into results/infer_bench_runs.csv to validate schema + appender.
    """
    env = collect_env()
...
def bench_run(
    ...
    """
    HF-only benchmark that writes infra-grade rows to results/infer_bench_runs.csv.
    One CSV row per measured run (so p50/p95 are computed across runs).
    """
    res = run_hf_bench(
        model=model,
